...
def apply():
    if _spenta():
        # il motivo vero l'ha gia' scritto _spenta(): qui non si inventa
        # una spiegazione fissa, che l'altra volta mi ha portato fuori strada
    ...
    try:
        ...
        if not hasattr(C, '_cerca_in_tabella_016'):
            ...
            return False
        # la funzione copiata scrive su file passando da qui: dentro
        # `exec` non vede le funzioni di questa patch, quindi gliela si
        # mette a disposizione come modulo.
        import sys as _sys
        import types as _types
        _mod = _types.ModuleType('karadom_traccia_029')
        _mod.riga = traccia
        _sys.modules['karadom_traccia_029'] = _mod
        C._orig_029 = C._sc_edit_brano
        spazio = m.__dict__
        exec(compile(CODICE, "<patch029>", "exec"), spazio)
        setattr(C, '_sc_edit_brano', spazio['_sc_edit_brano'])
        traccia('AGGANCIATA: _sc_edit_brano')

        # ⭐ NEL PROGRAMMA COMPILATO IL CAMPO PUO' ESSERE UN ALTRO.
        #    L'eseguibile del cliente e' piu' vecchio del sorgente: li' le
        #    righe della scaletta hanno ancora il loro Entry, configurato da
        #    `_setup_brano_autocomplete`. Nel sorgente di oggi quella
        #    funzione non la chiama piu' nessuno, ma nel compilato si', ed e'
        #    per questo che le correzioni sembravano non fare effetto: erano
        #    sull'altra strada. Qui si mette una spia su tutte e due, cosi'
        #    il log dice QUALE viene aperta davvero, senza doverlo indovinare.
        _viste = []
        for _nome in ('_setup_brano_autocomplete', '_sc_edit_field'):
            _fn = getattr(C, _nome, None)
            if _fn is None or getattr(_fn, '_spia029', False):
                continue

            def _con_spia(_orig=_fn, _etichetta=_nome):
                def dentro(self, *a, **k):
                    try:
                        traccia('campo APERTO da %s' % _etichetta)
                    except Exception:
                        pass
                    return _orig(self, *a, **k)
                dentro._spia029 = True
                return dentro

            setattr(C, _nome, _con_spia())
            _viste.append(_nome)
        if _viste:
            traccia('spia messa anche su: %s' % ', '.join(_viste))
