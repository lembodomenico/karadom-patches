...
def apply():
    ...
    try:
        ...
        if hasattr(C, 'carica_brani') and not hasattr(C, '_orig_016_carica'):
            C._orig_016_carica = C.carica_brani

            def carica_brani(self, cartella, forza=False,
                             _orig=C._orig_016_carica):
                esito = _orig(self, cartella, forza)
                try:
                    self.percorso_attivo = cartella
...
def revert():
    try:
        ...
        if hasattr(C, "_orig_016_carica"):
            C.carica_brani = C._orig_016_carica
            rimessi.append('carica_brani')
        if rimessi:
