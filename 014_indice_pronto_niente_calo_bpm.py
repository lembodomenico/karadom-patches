...
# ⚠️ LA CHIAMATA CI VUOLE. `hotfix.apply_all` fa soltanto
#       exec(compile(data, nome, "exec"), ns)
#    cioe' ESEGUE il file: non cerca `apply()` e non la chiama. Senza questa
#    riga la patch veniva caricata, il log diceva "[HOTFIX] applicata", e non
#    sostituiva un bel niente - trovato solo avviando col debug e vedendo che
#    mancava la riga "patch 014: ...". Tutte le altre patch la mettono in
#    fondo allo stesso modo.
try:
    apply()
except Exception as _e:
    print("patch 014: %s" % _e)
